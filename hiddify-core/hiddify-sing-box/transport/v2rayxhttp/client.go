...
func NewClient(ctx context.Context, dialer N.Dialer, serverAddr M.Socksaddr, options option.V2RayXHTTPOptions, tlsConfig tls.Config) (adapter.V2RayClientTransport, error) {
	...
	// If the outbound's TLS options didn't specify alpn explicitly (common for
	// subscriptions delivered as ready-made sing-box JSON, where nothing defaults
	// it the way ray2sing's vless://... URI builders do), default to offering h2
	// in the ClientHello. Without this, the TLS handshake completes but with no
	// ALPN negotiated, and decideHTTPVersion below then has nothing to go on
	// either -- XHTTP servers expect h2, so both sides end up disagreeing about
	// framing and the connection dies right after the handshake.
...
func decideHTTPVersion(tlsConfig tls.Config) string {
	...
	if len(tlsConfig.NextProtos()) == 0 {
		// No explicit alpn in the outbound's TLS options (common for subscriptions
		// that hand over a ready-made sing-box JSON config rather than a
		// vless://... link with an alpn= query param -- ray2sing's URI-based
		// builders default alpn to h2 themselves, this path doesn't). XHTTP
		// servers (Xray-core's XHTTP inbound, which is what this transport talks
		// to -- see server.go) expect HTTP/2 by default; silently falling back to
		// HTTP/1.1 here made every XHTTP connection over Reality/uTLS fail with
		// a handshake-adjacent "closed pipe" instead of actually negotiating.
		return "2"
	}
