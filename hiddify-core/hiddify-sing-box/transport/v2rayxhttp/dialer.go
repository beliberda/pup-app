...
func (c *DefaultDialerClient) OpenStream(ctx context.Context, url string, body io.Reader, uploadOnly bool) (wrc io.ReadCloser, remoteAddr, localAddr net.Addr, err error) {
	...
	go func() {
		...
		if err != nil {
			xhttpDebugLog("OpenStream(%s %s) http_version=%s error: %v", method, url, c.httpVersion, err)
		...
		if resp.StatusCode != 200 || uploadOnly { // stream-up
			if resp.StatusCode != 200 {
				xhttpDebugLog("OpenStream(%s %s) http_version=%s non-200 status: %s, headers: %v", method, url, c.httpVersion, resp.Status, resp.Header)
			}
...
func (c *DefaultDialerClient) PostPacket(ctx context.Context, url string, body io.Reader, contentLength int64) error {
	...
	if c.httpVersion != "1.1" {
		resp, err := c.client.Do(req)
		if err != nil {
			xhttpDebugLog("PostPacket(%s) http_version=%s error: %v", url, c.httpVersion, err)
			c.closed = true
			return err
		}
		if resp.StatusCode != 200 {
			xhttpDebugLog("PostPacket(%s) http_version=%s non-200 status: %s, headers: %v", url, c.httpVersion, resp.Status, resp.Header)
		}
		io.Copy(io.Discard, resp.Body)
