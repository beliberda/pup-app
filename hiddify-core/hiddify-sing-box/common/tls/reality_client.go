...
func (e *RealityClientConfig) ClientHandshake(ctx context.Context, conn net.Conn) (aTLS.Conn, error) {
	...
	// Xray-core >= 26.7.11 rejects REALITY clients below MinClientVer, which
	// defaults to 26.3.27 when the server operator leaves it unconfigured
	// (XTLS/Xray-core@af7eb68). These 3 bytes are the client version embedded
	// in the session ID that the server checks; this fork previously hardcoded
	// them to 1.8.1 (a ~2023 Xray-core version), so any current Xray-core
	// REALITY server silently rejects the handshake and falls back to
	// proxying the camouflage domain -- indistinguishable on the wire from a
	// real auth failure, but it's actually just this version gate.
	...
	realityDebugLog("ClientHandshake: serverName=%s pbk=%s shortID=%x sessionId=%x fingerprint=%s alpn=%v", e.uClient.ServerName(), base64.RawURLEncoding.EncodeToString(e.publicKey), e.shortID, hello.SessionId, e.uClient.id.Client, uConfig.NextProtos)

	err = uConn.HandshakeContext(ctx)
	if err != nil {
		realityDebugLog("ClientHandshake: HandshakeContext error (TLS handshake itself failed, VerifyPeerCertificate result if any is logged above): %v", err)
		return nil, err
	}
	...
	realityDebugLog("ClientHandshake: handshake completed without error, verified=%v negotiatedProto=%s tlsVersion=%x", verifier.verified, uConn.ConnectionState().NegotiatedProtocol, uConn.ConnectionState().Version)
...
func (c *realityVerifier) VerifyPeerCertificate(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
	...
	realityDebugLog("VerifyPeerCertificate: serverName=%s certCount=%d subject=%q issuer=%q pubKeyType=%T authKeyLen=%d", c.serverName, len(certs), certs[0].Subject.String(), certs[0].Issuer.String(), certs[0].PublicKey, len(c.authKey))
	if pub, ok := certs[0].PublicKey.(ed25519.PublicKey); ok {
		h := hmac.New(sha512.New, c.authKey)
		h.Write(pub)
		match := bytes.Equal(h.Sum(nil), certs[0].Signature)
		realityDebugLog("VerifyPeerCertificate: ed25519 leaf present, hmac match=%v", match)
		if match {
			c.verified = true
			return nil
		}
	} else {
		realityDebugLog("VerifyPeerCertificate: leaf pubkey is NOT ed25519 -- server did not return a reality-crafted cert, this looks like a real CA cert (server fell back to proxying the camouflage domain)")
	}
	...
	if _, err := certs[0].Verify(opts); err != nil {
		realityDebugLog("VerifyPeerCertificate: fallback real x509 verify failed: %v", err)
		return err
	}
	return nil
}
