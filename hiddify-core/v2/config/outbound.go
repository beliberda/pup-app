...
func patchOutboundTLSTricks(base option.Outbound, configOpt HiddifyOptions) option.Outbound {
	...
	if isOutboundReality(base) {
		// TLSFragment operates purely on the already-serialized ClientHello
		// bytes at the raw net.Conn.Write layer (splitting/delaying the TCP
		// writes), which happens *after* reality_client.go has finished
		// patching its auth token into those raw bytes -- so it's safe for
		// reality outbounds too. The other tricks below (mixed-case SNI,
		// uTLS padding) mutate actual TLS extension content, which would
		// conflict with reality's own uTLS-crafted ClientHello and its fixed
		// serverName matching, so those stay skipped for reality.
