...
func parseConfigContent(ctx context.Context, content []byte, debug bool, configOpt *HiddifyOptions, fullConfig bool) (*option.Options, error) {
	...
	if err := jsonDecoder.Decode(&tmpJsonResult); err == nil {
		...
		// NOTE: previously attempted to convert vless/vmess/trojan outbounds here
		// to the "xray" outbound type when UseXrayCoreWhenPossible is set, to cover
		// subscriptions that are already sing-box JSON (which skip
		// ray2sing.Ray2SingboxOptions entirely). Reverted: the "xray" outbound type
		// itself has no working implementation in this hiddify-sing-box fork --
		// protocol/hiddify/xray/outbound.go's New() unconditionally returns
		// E.New("Xray is not implemented yet"). Converting to it only turns a
		// protocol/transport this build genuinely can't handle (e.g. XHTTP) into a
		// guaranteed failure for every outbound, including ones that worked fine
		// natively. Revisit if/when xray-core is actually wired up upstream.
