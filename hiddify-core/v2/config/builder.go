...
func setInbound(options *option.Options, hopt *HiddifyOptions) {
	...
	if hopt.EnableTun {
		...
		opts := option.TunInboundOptions{
			// Upstream left this empty, letting the OS/sing-box assign a default
			// name (tun0/utun3/etc.) that naive VPN detectors regex-match against
			// (see PLAN.md §5.4, IndirectSignsChecker: patterns like
